...
the test harness), so by the time this function runs, a human has
already reviewed the draft and updated state with either approved=True
or human_feedback containing their notes.

route_after_generation decides where the graph goes next:
- approved -> END
- not approved, revisions remaining -> back to generate_rfc_draft
- not approved, max_revisions hit -> END anyway, draft ships as-is
"""
